...
def submit_question_to_s3(question_data: dict) -> bool:
    ...
    try:
        ...
        st.error(
            f":material/error: Missing configuration: {str(e)}. Please check your Streamlit secrets."
        )
        return False
...
)  # Cache for 5 minutes (admin-facing, less frequent updates)
...
def update_question_fields(
    s3_key: str, status: Optional[str] = None, response: Optional[str] = None
) -> bool:
    """
